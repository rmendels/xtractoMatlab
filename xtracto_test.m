function extractStruct = xtracto_test(datasetInfo, parameter, xpos, ypos, varargin)
...
% extract=xtracto(xpos, ypos, tpos,'phssta8day', xlen, ylen);
    ...
    addParameter(inputInfo, 'xlen', 0., @(x) iscellstr(x) || isnumeric(x)); 
    addParameter(inputInfo, 'ylen', 0., @(x) iscellstr(x) || isnumeric(x)); 
    addParameter(inputInfo, 'zlen', 0., @(x) iscellstr(x) || isnumeric(x)); 
