...
#' @return named 3-D array with the data, row names are the the longitudes of
#'   the data, column names are thel atitudes, and the third dimension names
#'   are the times.
#' @examples
#' extract<-xtracto_3D(xpos=c(230,240), ypos=c(40,45),
#'       tpos=c('2006-05-05','2006-06-21'), dtype=18)
#' extract<-xtracto_3D(xpos=c(230,240), ypos=c(40,45),
#'      tpos=c('2006-05-05','2006-06-21'), dtype='phssta8day')
#' @section Details
#'

xtracto_3D<-function(xpos, ypos, tpos, dtype){
  ...
  if(is.character(dtype)){
    ...
    erddapStruct<-getAllTimeBounds(erddapStruct)
...
downloadReturn<-getURL(myURL,fileout)
if(downloadReturn != 0){
   print(myURL)
...
if (max(xpos) > 180.){
   longitudes<-make360(datalongitude)
}
#request is on (-180,180)
if (min(xpos) < 0.){
  longitudes<-make180(datalongitude)
}
